...
@section('content')
    ...
    <div class="container p-2 mx-auto">
        <div class="flex flex-row flex-wrap py-4">
            <aside class="w-full sm:w-1/3 md:w-1/4 px-2 mt-8">
                <div class="sticky top-4 p-4 rounded-lg w-full border-2 border-yellow-400 shadow-xl rounded-xl text-gray-800">
                    <ul class="space-y-1">
                        @foreach($relatedPages as $relatedPage)
                            <li>
                                <a href="{{$relatedPage->slug}}" class="flex items-center p-2 text-base text-gray-900 rounded-md dark:text-gray-200 hover:bg-yellow-300 dark:hover:bg-gray-700 dark:hover:text-gray-200 group">
                        ...
                        @if(count($relatedLinks) > 0)
                            ...
                            @foreach($relatedLinks as $relatedLink)
                                <li>
                                    <a href="{{$relatedLink->url}}" class="flex items-center p-2 text-base text-gray-900 rounded-md dark:text-gray-200 hover:bg-yellow-200 dark:hover:bg-gray-700 dark:hover:text-gray-200 group">

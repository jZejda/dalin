            ...
            <div class="grid pt-2 text-left border-t border-gray-200 md:gap-16 dark:border-gray-700 md:grid-cols-3">
                <div>
                    <div class="mb-10">
                        ...
                        <div class="px-2">
                            @if ($record->date !== null)
                                <div class="flex items-center space-x-2 mb-3 pb-2 border-b border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400">
                                    ...
                                    <span class="text-sm">{{ $record->date->format(\App\Shared\Helpers\AppHelper::DATE_FORMAT) }}</span>
                                </div>
                            @endif

                            @if ($record->entry_date_1 !== null || $record->entry_date_2 !== null || $record->entry_date_3 !== null)
                                <div class="flex flex-wrap gap-2">
                                    @if ($record->entry_date_1 !== null)
                                        <x-sport-event.entry-date-tile
                                            :date="$record->entry_date_1"
                                            :number="1"
                                        />
                                    @endif
                                    @if ($record->entry_date_2 !== null)
                                        <x-sport-event.entry-date-tile
                                            :date="$record->entry_date_2"
                                            :number="2"
                                            :fee="$record->increase_entry_fee_2"
                                        />
                                    @endif
                                    @if ($record->entry_date_3 !== null)
                                        <x-sport-event.entry-date-tile
                                            :date="$record->entry_date_3"
                                            :number="3"
                                            :fee="$record->increase_entry_fee_3"
                                        />
                                    @endif
                                </div>
                            @endif
